                  ...
                  <AnimatePresence>
                    {weeklyData.map((item, index) => (
                      ...
                      >
                        ...
                        <span className="text-sm font-medium text-blue-700">{item.water}л</span>
                      </motion.div>
                    ))}
                  </AnimatePresence>
                </div>
              ...
              <div className="flex items-end h-[120px] gap-2 relative">
                <TooltipProvider>
                  {isLoading ? (
                    ...
                    history.map((day, i) => {
                      const percentage = (day.value / (Number(hydraGoal) * 1000 || goal)) * 100;
                      const barColor = getBarColor(percentage);

                      return (
                        <div key={i} className="flex-1 flex flex-col items-center pl-5">
                          <Tooltip>
                            ...
                            <TooltipContent side="top" className="bg-blue-800 text-white border-blue-900 shadow-xl">
                              <div className="text-center p-1">
                                <p className="font-medium">{(day.value / 1000).toFixed(1)}л</p>
                                <p className="text-xs text-blue-200">{percentage.toFixed(0)}% от цели</p>
                                {percentage >= 100 && (
                          ...
                          <span className="text-xs mt-2 font-medium text-gray-600">{day.date}</span>
                        </div>
                      );
                    })
                  )}
