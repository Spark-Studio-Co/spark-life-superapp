...
import type { RegisterFormData } from "../multistep-form"
...
export const StepPersonalInfo = ({ formik }: StepPersonalInfoProps) => {
  ...
  const passwordStrengthText = ["Очень слабый", "Слабый", "Средний", "Хороший", "Сильный"]
  const passwordStrengthColor = ["bg-red-400", "bg-orange-400", "bg-yellow-400", "bg-lime-400", "bg-green-500"]

  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-bold text-gray-800">Личные данные</h2>
        <p className="text-gray-500 mt-1 text-sm">Введите ваши личные данные для создания аккаунта</p>
      </div>
      ...
      <div className="space-y-1">
        ...
        <AnimatePresence>
          {formik.values.password && (
            <motion.div className="space-y-1" initial="hidden" animate="visible" exit="exit" variants={errorVariants}>
              <div className="flex gap-1 h-1">
                {[0, 1, 2, 3, 4].map((index) => (
                  <motion.div
                    ...
                    className={`h-full flex-1 rounded-full transition-all ${index < passwordStrength ? passwordStrengthColor[passwordStrength - 1] : "bg-gray-200"
                      }`}
                  />
                ))}
              </div>
              <motion.p className="text-xs text-gray-500">
                Надежность пароля:{" "}
                <span className="font-medium">{passwordStrengthText[passwordStrength - 1] || "Очень слабый"}</span>
              </motion.p>
            </motion.div>
          )}
        </AnimatePresence>
